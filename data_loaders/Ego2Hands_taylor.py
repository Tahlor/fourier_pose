...
class Ego2HandsData(data.Dataset):
    ...
    def set_B_gauss(self, B_gauss=None, mapping_size=32):
        if B_gauss is None:
            self.B_gauss = np.random.normal((mapping_size, 2))
        else:
    ...
    def __getitem__(self, index):
        if not self.args.eval:
            ...
            coords = self.get_middle_point(heatmaps.transpose(2, 0, 1))
            coords_proj = np.matmul(coords,self.B_gauss.T) # 21x2 , 2x32 -> 21 x 32
            proj = tensor(coords_proj).type(torch.float)
            sin_cos = torch.stack([torch.sin(proj*2*np.pi),torch.cos(proj*2*np.pi)], dim=2)
            invalid = np.isnan(coords).any()
            return {"image": img_input_tensor,
                    "heat": heatmaps_tensor,
                    "coords": tensor(coords).type(torch.float),
                    "proj": proj,
                    "sin_cos": sin_cos,
                    "valid": not invalid,
                    "index": index}
    ...
    def get_middle_point(self, gt):
        """ gt: tensor, 22,28,28

        """
        gt = gt[1:]
        x = np.linspace(0, 1, gt.shape[1], endpoint=False)
        y = np.linspace(0, 1, gt.shape[2], endpoint=False)
        (X, Y) = np.meshgrid(x, y)
        x_coord = (X * gt).sum(axis=(1, 2)) / gt.sum(axis=(1, 2)).astype("float")
        y_coord = (Y * gt).sum(axis=(1, 2)) / gt.sum(axis=(1, 2)).astype("float")
        return np.c_[x_coord, y_coord]

        # x = np.argmax(np.sum(gt, axis=1), axis=1)
        # y = np.argmax(np.sum(gt, axis=2), axis=1)
        #
...
if __name__=='__main__':
    import shlex
    command = "--save_outputs --config=../configs/config_ego2hands.yml" #  --data=data/ffhq-1024x1024.zip --gpus=1 --batch=32 --gamma=8.2 --mirror=1

    args = shlex.split(command)
    args = parse(args)

    config = Config(args.config)
    hand_dataset = Ego2HandsData(args, config)
    img, gt = next(iter(hand_dataset))
    # [ [3,224,224], [22,28,28] ]
